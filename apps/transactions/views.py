...
class NewTransaction(generic.FormView):
    """
    Class to Register a Transaction
    """
    ...
    def form_valid(self, form):
        try:
            self.newtransaction = form.save(commit=False)
            self.newtransaction.operator = self.request.user
            
            self.newtransaction.save()
